...
export async function POST(request: Request) {
  ...
  try {
    ...
    const orderResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/vignette_orders?order_number=eq.${encodeURIComponent(orderNumber)}&select=id,order_number,customer_email,total_amount,currency,payment_status,status`,
    ...
    if (!order.total_amount || !order.currency || order.currency === "MULTI") {
      return NextResponse.json({ error: "Jedna płatność może obecnie obejmować tylko winiety w tej samej walucie." }, { status: 400 });
    }

    const itemsResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/vignette_order_items?order_id=eq.${encodeURIComponent(order.id)}&select=product_name,validity,registration_number,final_price,currency`,
      { headers: dbHeaders, cache: "no-store" }
    );
    ...
    items.forEach((item: { product_name: string; validity: string; registration_number: string; final_price: number; currency: string }, index: number) => {
      const amount = Math.round(Number(item.final_price) * 100);
    ...
    const patchResponse = await fetch(`${SUPABASE_URL}/rest/v1/vignette_orders?id=eq.${encodeURIComponent(order.id)}`, {
      ...
      body: JSON.stringify({ payment_status: "processing", payment_provider: "stripe", payment_reference: stripeData.id }),
    });
    if (!patchResponse.ok) console.error("Could not update payment session on order", await patchResponse.text());
