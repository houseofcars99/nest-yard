...
export default function HomePage() {
  ...
  const availableProducts = useMemo(() => vignetteProducts.filter((item) => item.country === countryCode && item.vehicleType === vehicle), [countryCode, vehicle]);
  ...
  function addToCart() {
    ...
    if (showStartDate && startDate < today) {
      setError("Data rozpoczęcia nie może być wcześniejsza niż dzisiaj.");
      return;
    }
    if (!product) return;
    ...
    <main className="vignette-page">
      ...
      {showCart && (
        <div className="cart-overlay" onClick={() => setShowCart(false)}>
          <aside className="cart-drawer" onClick={(event) => event.stopPropagation()}>
            <div className="drawer-head"><div><span className="label">TWOJA PODRÓŻ</span><h2>Koszyk</h2></div><button className="close-button" onClick={() => setShowCart(false)} aria-label="Zamknij">×</button></div>
            {cart.length === 0 ? <p className="empty-cart">Koszyk jest pusty. Dodaj pierwszą winietę.</p> : <>
      ...
      <section className="hero">
        ...
        <div className="purchase-card">
          ...
          <div className="field-row"><label><span>Typ pojazdu</span><select value={vehicle} onChange={(event) => setVehicle(event.target.value as "car" | "motorcycle")}><option value="car">Samochód</option><option value="motorcycle">Motocykl</option></select></label><label><span>Kraj rejestracji</span><select value={registrationCountry} onChange={(event) => setRegistrationCountry(event.target.value)}>{registrationCountries.map((code) => <option key={code} value={code}>{code}</option>)}</select></label></div>
          <label className="full-field"><span>Numer rejestracyjny</span><input value={registrationNumber} onChange={(event) => setRegistrationNumber(event.target.value.toUpperCase())} placeholder="np. DW 12345" /></label>
          {showFuel && <label className="full-field"><span>Rodzaj napędu</span><select value={fuel} onChange={(event) => setFuel(event.target.value)}><option value="diesel">Diesel</option><option value="petrol">Benzyna</option><option value="electric">Elektryczny / wodór</option><option value="biomethane">Biometan</option><option value="cng">CNG / LNG</option><option value="plug-in">Plug-in hybrid</option></select></label>}
          <label className="full-field"><span>Okres ważności</span><select value={product?.validity ?? validity} onChange={(event) => setValidity(event.target.value)}>{availableProducts.map((item) => <option key={item.id} value={item.validity}>{item.validity}</option>)}</select></label>
          {showStartDate && <label className="full-field"><span>Data rozpoczęcia</span><input type="date" min={today} value={startDate} onChange={(event) => setStartDate(event.target.value)} /></label>}
