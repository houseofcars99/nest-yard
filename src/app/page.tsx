...
type CartItem = { id: string; country: Country; validity: string; registrationNumber: string; finalPrice: number; currency: string };
...
export default function HomePage() {
  ...
  function addToCart() {
    if (!registrationNumber.trim()) { setError("Wpisz numer rejestracyjny."); return; }
    if (showStartDate && !startDate) { setError("Wybierz datę rozpoczęcia."); return; }
    if (!product) return;
    setCart((items) => [...items, { id: `${product.id}-${Date.now()}`, country: selectedCountry, validity: product.validity, registrationNumber: registrationNumber.trim(), finalPrice: price, currency: product.currency }]);
    setError(""); setShowCart(true);
  }
  function removeFromCart(id: string) { setCart((items) => items.filter((item) => item.id !== id)); }
  ...
  return (
    <main className="vignette-page">
      ...
      {showCart && <div className="cart-overlay" onClick={() => setShowCart(false)}><aside className="cart-drawer" onClick={(e) => e.stopPropagation()}><div className="drawer-head"><div><span className="label">TWOJA PODRÓŻ</span><h2>Koszyk</h2></div><button className="close-button" onClick={() => setShowCart(false)} aria-label="Zamknij">×</button></div>{cart.length === 0 ? <p className="empty-cart">Koszyk jest pusty. Dodaj pierwszą winietę.</p> : <>{cart.map((item) => <div className="cart-item" key={item.id}><div><strong>{item.country.flag} {item.country.name}</strong><span>{item.validity} · {item.registrationNumber}</span></div><div><b>{money(item.finalPrice, item.currency)}</b><button onClick={() => removeFromCart(item.id)}>Usuń</button></div></div>)}<div className="cart-total"><span>Łącznie</span><strong>{money(cartTotal, cart[0].currency)}</strong></div><button className="primary-button checkout-button" disabled={!cart.length}>Przejdź do płatności →</button><p className="price-note">Cena końcowa. Bez rozbijania ceny na dodatkowe składniki.</p></>}</aside></div>}
